...
impl Relay {
    ...
    pub fn shutdown(self) -> Result<()> {
        ...
        loop {
            let shutdown_res = self.shutdown_tx.send(());
            match shutdown_res {
                Ok(()) => {
                    break;
                }
        ...
        assert!(Self::port_is_available(self.port));
        Ok(())
    }

    fn get_available_port() -> Option<u16> {
        let startsearch = Self::PORT_COUNTER.fetch_add(10, Ordering::SeqCst);
        if startsearch >= 20000 {
            // wrap around
            Self::PORT_COUNTER.store(4030, Ordering::Relaxed);
        }
        (startsearch..20000).find(|port| Self::port_is_available(*port))
    }
