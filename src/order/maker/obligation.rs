  ...
  pub fn to_tags(&self) -> HashSet<String> {
    ...
    match self {
      ...
      ObligationKind::Fiat(currency, payment_methods) => {
        ...
        tag_string_set = HashSet::from([prefix_string.to_string(), currency_prefix_string.to_lowercase()]);
      ...
      ObligationKind::Custom(obligation_string) => {
        ...
        tag_string_set = HashSet::from([prefix_string.to_string(), format!("{}-{}", prefix_string, obligation_string).to_lowercase()]);
      }
    }
    tag_string_set
  }
...
mod tests {
  ...
  fn bitcoin_onchain_obligation_kind_to_tags() {
    let obligation_kind = ObligationKind::Bitcoin(HashSet::from([BitcoinSettlementMethod::Onchain]));
    let obligation_tags = obligation_kind.to_tags();
    let expected_tags = HashSet::from(["ob-bitcoin-onchain".to_string(), "ob-bitcoin".to_string()]);
    print!("Obligation: {:?} Expected: {:?}", obligation_tags, expected_tags);
    assert_eq!(obligation_tags, expected_tags);
  }

  #[test]
  fn fiat_usd_venmo_obligation_kind_to_tags() {
    let obligation_kind = ObligationKind::Fiat(Currency::USD, HashSet::from([FiatPaymentMethod::Venmo, FiatPaymentMethod::CashApp]));
    let obligation_tags = obligation_kind.to_tags();
    let expected_tags = HashSet::from(["ob-fiat-usd-venmo".to_string(), "ob-fiat-usd-cashapp".to_string(), "ob-fiat-usd".to_string(), "ob-fiat".to_string()]);
    print!("Obligation: {:?} Expected: {:?}", obligation_tags, expected_tags);
    assert_eq!(obligation_tags, expected_tags);
  }
  ...
  fn custom_obligation_kind_to_tags() {
    ...
    let obligation_tags = obligation_kind.to_tags();
    let expected_tags = HashSet::from(["ob-custom-barter".to_string(), "ob-custom".to_string()]);
    print!("Obligation: {:?} Expected: {:?}", obligation_tags, expected_tags);
    assert_eq!(obligation_tags, expected_tags);
  }
}